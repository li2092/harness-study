...
2025年12月我开始利用Claude Code尝试开发harness，那时候还没这个词，我管我手上的项目叫Agent驱动的XX产品。我第一个项目是Cyber-Mantic，给LLM配上工具让它能准确算出各个术数理论的结果来进行分析，当时agent runtime直接把OpenCode给塞进去了。这个项目的效果不及预期，但给我了一个很强烈的信号：LLM增强了工具调用能力后，配合上agent runtime是未来几年最重要的一个AI工具。于是我尝试从0开始搭建智能体，不借助现有的framework，尝试把所有可能的形态都试一遍。于是我利用每天娃睡了的时间乐此不疲的踩着各种harness的坑、vibe coding到凌晨持续了快4个月。

随着我在小红书发帖小小的火了一下，我和许多主包一同分享交流harness的经验时发现我积累的一些小技巧对他们很有帮助，于是萌生了系统性的写一个教程的想法。在写作的过程中，有的朋友拿到了实习Offer、有的朋友萌生了创业的想法，而最有行动力的一个老哥，已经开始开发产品融到了第一笔资金，我当然也没闲着落地了几个企业级的垂域agent产品，进而多方位验证了我对harness理解的正确性，不学术但很工程。

我希望《harness study》能帮助大家更系统、更深入的理解什么是harness，让AI coding工具拿到这份教程、根据用户的需求描述就能生成一个够用的Agent产品。同时我也希望能推动harness本土化的翻译过程，只有当harness的每个细节进行更准确、更统一的定义，才能让更多人理解这个概念，才能逐步在各行各业发光发热。

接下来我会：
- 进一步对Introduction部分进行校对；
- 完善《Harness-Lab》的工程细节；
- 结合《harness study》和《Harness-Lab》的内容进行工程实践，打造一个专门为Deepseek V4适配的Agent；
- 将工程过程沉淀成《harness study》的展开章节。
