...
<p align="center">
  ...
  <img alt="实战笔记卷" src="https://img.shields.io/badge/实战笔记卷-14章-5b21b6?style=flat-square">
...
本项目（Harness Study）就是为了把围绕模型的这一层作为独立的工程对象，系统化地讲清楚。**项目分卷展开**：当前已写完**入门卷**（一次性走完全骨架）与**架构与工程卷**（第二卷：把这套部件组装成语义正确、可中断、可恢复、可验证的运行时）；**第三卷**（生产工程：一次执行正确之后，怎样让一套服务长期可靠）正在写作。另有一册横切全书的**实践卷**《Harness 实战笔记》，不占主线卷号。
...
- ⏳ **第三卷 · 生产工程**：卷级 spec 已定（四变量 → 五类契约 → 二十章问题链），正文写作中。
- ✓ **实践卷 · Harness 实战笔记**：横切全书、不占主线卷号。14 章 + 104 条目 + 41 配图完稿，落入 [`field-notes/`](field-notes/)。
...
架构与工程卷（第二卷）讲部件之后的下一步：把八种 runtime 机制组装成一个**语义正确、可中断、可恢复、可验证**的运行时。生产工程（数据库运维、安全治理、发布、SRE、成本）在第三卷，共二十章。
...
- 上下文管理——决定模型每一轮看到什么（第三、四、五、七章）
- 释放能力上限——不要用不必要的限制削减模型本来能做到的事（第二章）
- 可控的环境——边界由代码保证，不靠 prompt 请求（第六、八、九、十、十一章）

第一章把一条消息的处理过程展开成四阶段十三步骤，后续各章的故障都能定位到具体的步骤编号；第十二到十四章是判断这三件事做得好不好的手段。
...
| 章 | 主题 | 回答的问题 |
|---|---|---|
| §一 | 一条消息经过 harness 的全过程 | harness 在做什么 |
| §二 | 预算与限制 | 哪些配置在削减模型的能力 |
| §三 | 压缩 | 上下文装不下时留什么 |
| §四 | 工具定义与参数处置 | 模型怎样才会正确调用工具 |
| §五 | 模型协议与输出解析 | 协议层能吸收多少差异 |
| §六 | 循环控制与空转检测 | 怎么让它停下来 |
| §七 | 提醒注入与交付可见性 | 怎么让信息在该出现的时刻出现 |
