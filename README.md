...
本项目（Harness Study）就是为了把围绕模型的这一层作为独立的工程对象，系统化地讲清楚。**项目分卷展开**：当前已写完**入门卷**（一次性走完全骨架）与**架构与工程卷**（第二卷：把这套部件组装成语义正确、可中断、可恢复、可验证的运行时）；第三卷（生产工程与治理）在大纲阶段。另有一册横切全书的**实践卷**《Harness 实战笔记》，不占主线卷号。
