...
（配图占位：双层控制流图——上层 orchestration 决定 spawn 谁、等谁、怎么合，下层节点内的 agentic loop 决定调不调工具、何时停；两层之间标出各自的正确性判据。）
...
Anthropic 给过一条决策梯度：单次模型调用→workflow→单 agent→多 agent，每上一级，都用更高的延迟、成本和复合错误风险，去换更强的表现【厂商实践】。它自己的多 agent 研究系统给出一组厂商自评数据：在偏检索的研究任务上，一个 Opus lead 带几个 Sonnet subagent，比单个 Opus 高 90.2%；代价在 token 上——单 agent 约为普通对话的 4 倍，多 agent 约为 15 倍；同一基线折算下来，多 agent 约为单 agent 的 3.75 倍。而且 token 用量本身就解释了 80% 的表现差异【厂商实践；四个数字终稿回核 multi-agent-research-system 原文，非独立复现基准】。这笔账的关键在读写不对称：并行地读可以扩展，并行地写会制造互相冲突的决策，所以收敛那一步必须落回单点——LangChain 把这条讲明，Anthropic 在实现里就是让检索并行、最终综合交给单个 agent【厂商实践】。

反过来的证据也在积累。已经有预印本报告，把推理 token 预算控住之后，单 agent 在多跳推理任务上一致地持平或反超多 agent；同一篇也划了边界——当单 agent 的有效上下文利用被削弱、或者愿意多投算力时，多 agent 就重新有竞争力【预印，arXiv:2604.02460，限多跳推理，量级参考、终稿回核】。业界的对垒立场也应并陈：Cognition 在《Don't Build Multi-Agents》里主张单线程＋上下文压缩，与 Anthropic 的 orchestrator-worker 正面相左——两条路线各有适用边界，不预设谁对谁错【厂商实践，Cognition／Anthropic 立场对垒，措辞终稿回核】。所以判据要冷一点：多 agent 用成本换质量，扩展从来不免费——只有当任务能切成互相独立、以读为主的子任务，且收敛点单一时才值。代价直白：一旦上多 agent，token 账就抬到单 agent 的约 3.75 倍，外加一整套协调开销；止损线画错，多出来的钱全花在协调上，落不到质量里。
...
所以设计上，subagent 回传默认不可信，经独立校验才采信。业界的校验层已经长出好几种形态：OpenAI 的 guardrails 与 agent 执行并行跑、一旦触发 tripwire 立即中断【规范/官方文档】；Anthropic 的研究系统单开一步独立的 citation pass 专门核对引用【厂商实践】；最省的一层是确定性检查——schema、长度、安全过滤，零成本地全量兜底。这些都是为了绕开"内部即可信"。反面的样子在产品里同样现成：Cognition 的 managed Devins 是每个 child 自己跑测试、自己核过改动再回报，主 Devin 只做 coordinator 收拢结果【厂商实践】——检查器和被检查者是同一个。关键一条接回第十一章 11.5 节：校验器和被校验者不能共享同一个判断源，否则那个共享件一坏，校验和产出一起错，又是一次共因失效。

即便如此，验证仍是多 agent 里最弱的一环——MAST 里验证类失效独占 23.5%，是一类独立、且长期被低估的失败。代价压在成本上：校验是又一次模型调用，全量校验贵、抽样校验漏；判官本身也会偏（对高置信的错误答案尤其容易放行），覆盖率和成本之间没有免费的解。
