...
这个直觉有大样本的反证。UC Berkeley 的 MAST 研究分析了 7 个主流多 agent 框架的 1600 多条标注轨迹，把失败归成三大类：系统设计与规格问题占 41.8%、agent 之间的错位占 36.9%、验证失败占 21.3%【评审（NeurIPS 2025 接收状态终稿回核，未接收则降标为预印），arXiv:2503.13657；三占比与样本量终稿回核】。前两类加起来近八成：多 agent 的失效主要出在 agent 之间的接口和边界上，单个 agent 的能力只是次要因素。最典型的一种叫 infinite handoff loop：任务在 A→B→C→A 之间转圈，没有哪个 agent 真正认领它，每转一手都重新规划一次，上下文在每一跳里漏掉一截。
...
Anthropic 给过一条决策梯度：单次模型调用→workflow→单 agent→多 agent，每上一级，都用更高的延迟、成本和复合错误风险，去换更强的表现【厂商实践】。它自己的多 agent 研究系统给出一组厂商自评数据：在偏检索的研究任务上，一个 Opus lead 带几个 Sonnet subagent，比单个 Opus 高 90.2%；但代价是约 15 倍的 token（单 agent 已经是普通对话的约 4 倍），而且 token 用量本身就解释了 80% 的表现差异【厂商实践；四个数字终稿回核 multi-agent-research-system 原文，非独立复现基准】。这笔账的关键在读写不对称：并行地读可以扩展，并行地写会制造互相冲突的决策，所以收敛那一步必须落回单点——LangChain 把这条讲明，Anthropic 在实现里就是让检索并行、最终综合交给单个 agent【厂商实践】。

反过来的证据也在积累。已经有预印本报告，在同等思考 token 预算下，单 agent 常常反超多 agent【预印，arXiv:2604.02460/2604.27891，量级参考、终稿回核】。业界的对垒立场也应并陈：Cognition 在《Don't Build Multi-Agents》里主张单线程＋上下文压缩，与 Anthropic 的 orchestrator-worker 正面相左——两条路线各有适用边界，不预设谁对谁错【厂商实践，Cognition／Anthropic 立场对垒，措辞终稿回核】。所以判据要冷一点：多 agent 用成本换质量，扩展从来不免费——只有当任务能切成互相独立、以读为主的子任务，且收敛点单一时才值。代价直白：一旦上多 agent，就把 15 倍的 token 和一整套协调开销一起买了下来；止损线画错，多出来的钱全花在协调上，落不到质量里。

## 12.4 父子之间：权力取交集，不取并集

值得上了，第一个要守的契约是权限——它把第十一章的规则从单 agent 推到父子链。

"契约沿父链继承"在权限这一格有个精确形态：有效权限＝agent 的权限 ∩ user 的权限——两边都放行才放行，取的是交集。子 agent 默认收窄，且绝不自动继承父的"全放行"——第十一章那个 Lingering Authority 的反例（subagent 继承父 bypass 造成权限蔓延）说的就是取错了方向。取并集的危险已有公开演示坐实：2025 年 9 月有人演示过 Cross-Agent Privilege Escalation，两个 agent 互相改写对方的配置、彼此把权限一点点抬高，形成自我强化的污染【预印/研究记要，出处与等级终稿回核】。失效根因是把委派当成了权力的透传——委派本该收窄，透传却把权力原样放大，每一次委派都成了一个 confused deputy（被利用的代理人）。

正面形态在 2026 的产品里已经能看到。Codex 的子 agent 继承的是父会话那套收窄的沙箱和网络规则——继承来的是限制本身【厂商实践，v0.115.0，终稿回核版本】；Claude Code 在 spawn 子 agent 之前先把它要用的工具权限逐一批一遍，运行时只认预批过的、其余一律自动拒绝【规范/官方文档】。两家都把"收窄"钉在了委派那一刻，没留给子 agent 自觉。收窄也有代价，和第十一章同一处权衡：交集取得太狠，child 常常权不够、每做一步都回父要权，任务被一串升级请求切碎。刻度还是那句——卡在"完成这个子任务所需的最小集"，一味求窄反而添乱。

## 12.5 几个分身抢写同一份文件

权限守住了"谁能动"，下一个问题是"一起动同一样东西"——几个 child 并行，抢写同一份文件或同一块状态。

2026 有一个几乎通用的答案：git worktree 隔离。Codex、Devin、Cursor 都这么做——每个 child 拿一个独立的工作目录和分支，共享同一个 `.git` 对象库，执行期间各改各的、互不冲突【厂商实践】。但要看清它的边界：隔离不等于协调。worktree 只是把冲突推迟到 merge 那一刻，不是消灭冲突。真正共享一块状态的并发写，得有显式的合并语义。LangGraph 把这条做得最成型：两个节点在同一步里写同一个 state key，会直接抛 `INVALID_CONCURRENT_GRAPH_UPDATE`，逼你写一个 reducer 函数告诉图该怎么合并，checkpointer 再用基准 checkpoint 的 ID 做乐观并发检测【规范/官方文档】。这套也有它的失效点：高吞吐下会触发激进的重试抖动，把 I/O 和模型调用的成本一起耗掉。

失效根因是把并行写当成了并行读——以为各写各的没事，实际两个写决策无声地互相覆盖。设计出场顺着第六章的单一写者和第八章的幂等来：写路径要么单点化，要么用显式 reducer 合并；worktree 负责执行期隔离，父 run 在 merge 这个唯一的收敛点上做仲裁。代价是隔离换来了执行期零冲突，也换来了"没有共享的真相"——worktree 里的 child 彼此看不见，所有协调都压到父身上，父于是成了瓶颈。这条代价在下一节还会再现一次。

## 12.6 一个子任务失败，整批怎么办

隔离解决了并发写，可一批并行的 child 里总会有人失败——这一批的结果怎么收敛，是下一张欠条。

先看框架给到哪一步。LangGraph 的 superstep 是事务性的：一批并行分支里任何一个抛异常，整个 superstep 的状态更新全部不落盘；配套的 RetryPolicy 只重试失败的那个分支，而且每次重试前先清掉这个 task 已写的东西，免得失败尝试的半截写污染状态【规范/官方文档】。这解决了"失败别留脏数据"。但它没解决"部分成功怎么办"。Anthropic 的研究系统在这里留了个诚实的坑：lead agent 是同步等一批 subagent 全跑完才继续的，一个慢 subagent 会阻塞整批，中途也没法纠偏【厂商实践】——这是半途设计的真实样子：这一版就到这儿。

缺口于是清楚了：带部分结果的优雅降级，框架基本不给，仍是应用层的活。谁来收敛、收敛成什么、几个成功才算够，是父的职责。设计上父要显式定义收敛判据——全成功、多数成功、还是关键子任务成功——和对应的失败处置：重试、降级、还是整批放弃。这一步能接回前面两条老纪律：收敛的终结原因落第五章那四类（业务失败/基础设施失败/用户取消/策略拒绝），而"没跑完、结果未知"是第八章立过的合法终态 unknown，不许伪造成成功。代价是个两难：事务性收敛意味着一个 child 失败可能让整批白跑，可放宽成"部分交付"又可能把不完整的结果当成品用；在"整批原子"和"部分可用"之间怎么选，得按任务定，选错一头是浪费、另一头是脏数据。
...
所以设计上，subagent 回传默认不可信，经独立校验才采信。业界的校验层已经长出好几种形态：OpenAI 的 guardrails 与 agent 执行并行跑、一旦触发 tripwire 立即中断【规范/官方文档】；Anthropic 的研究系统单开一步独立的 citation pass 专门核对引用【厂商实践】；Devin 干脆设了一类专职的 verifier agent，拿产出去对齐 spec【厂商实践】；最省的一层是确定性检查——schema、长度、安全过滤，零成本地全量兜底。这些都是为了绕开"内部即可信"。关键一条接回第十一章 11.5 节：校验器和被校验者不能共享同一个判断源，否则那个共享件一坏，校验和产出一起错，又是一次共因失效。

即便如此，验证仍是多 agent 里最弱的一环——MAST 里验证类失效独占 21.3%，是一类独立、且长期被低估的失败。代价压在成本上：校验是又一次模型调用，全量校验贵、抽样校验漏；判官本身也会偏（对高置信的错误答案尤其容易放行），覆盖率和成本之间没有免费的解。
...
六格问完，问不出第七格。这不靠"数满六个"的运气，靠一条穷尽性根据：多 agent 没有引入新的失效种类。MAST 那三大类失效（规格设计、inter-agent 错位、验证失败）逐一落回既有契约维度——规格错落到转移与真相、错位落到交互与权限、验证漏落到证据与副作用，没有哪一类要求前十一章没立过的新维度。失效空间没有扩张，契约集合就不必扩张。多 agent 要守的，还是这六份，只不过每一份都递归地套在了父子结构上。**这就是"契约沿父链继承"的全部含义：多 agent 不是一套新规则，是老规则多套了一层。** 谁把这层看成新物种，谁就会去发明本不需要的新机制；谁看清它是递归，谁就知道该复用前十一章立好的每一件东西。父子 run 怎么继承这六份契约、协调专属的收敛判据与输出信任各落在哪，登记为工作制品 W（Parent-Child Run Contract，父子 run 契约表）。
...
四问落位：可观测由父子 correlation 与收敛/校验事件兑现；可控是本章主场——权限取交集、收敛点单点写、回传默认不可信，三格全押在"多 agent 是六契约的递归，必须管控"上；稳定兑现在隔离与 partial failure 收敛——一个 child 失败不污染整批状态（superstep 事务性）、慢 child 不无限阻塞（收敛判据超时）；闭环的一环在回传校验上成形——触发点是 child 回传本身，校验（检测）比对产出与 spec/schema 是否自洽，不合则拒、并按 12.6 的失败处置回退到重试或降级（纠正——回到好终态才算闭环），采信或拒绝都落事件（验证）。此环在配套实现里目前只到设计，回传校验的落账尚未接齐（见本节 L 级自检），四问描述的是机制该有的样子，不是桌面案例已兑现的样子。
...
多 agent 的六契约守住了：child 是 run、权限取交集、并发单点写、失败事务性收敛、回传独立校验。可这一切——契约真的沿父链继承了吗、并发真的单点收敛了吗、回传真的被校验了吗——怎么证明它确实发生过？child 的 trace 怎么跟父串起来、跨 agent 的因果链断在哪一跳怎么查？前面每一章都在往证据面记账，现在该把这本账本本身立起来了。Evidence Plane，第十三章。
...
| 终稿回核清单 | — | Claude Code dynamic workflow 发布日期 2026-05-28／触发词改 ultracode 日期 06-03／CLI 版本（回核 Anthropic 官方）；Codex v0.115.0 子 agent 继承收窄沙箱＋网络的版本号与日期（回核发布说明）；Devin coordinator/implementor/verifier agent 类型（回核 Cognition 官方）；Cross-Agent Privilege Escalation 出处与"取交集"建议来源（CSA research note / WorkOS，拆两级）；单 agent 反超多 agent arXiv:2604.02460/2604.27891 编号与结论；MAST arXiv:2503.13657 NeurIPS 2025 接收状态＋三占比与 κ；Anthropic +90.2%/15x/80% 方差回核 multi-agent-research-system 原文；MS "能写函数就别用 agent" 出处（learn.microsoft.com/agent-framework）；Cognition "Don't Build Multi-Agents" 若直引需回原帖措辞 |

## 引用来源

> （编辑工作区：本节出版前整体删除。）

- 【评审】UC Berkeley MAST（arXiv:2503.13657，NeurIPS 2025）——7 框架 1600+ trace、κ=0.88、14 失效模式三大类（系统设计/规格 41.8%、inter-agent 错位 36.9%、验证 21.3%）、infinite handoff loop（12.1/12.7；终稿回核占比与编号）。
- 【厂商实践】Anthropic — How we built our multi-agent research system（orchestrator-worker、+90.2%、约 15x/4x token、token 解释 80% 方差、独立 citation pass、lead 同步执行瓶颈）（12.3/12.6/12.7）。指针 research/volume2/03。终稿回核原文数字。
...
- 【厂商实践】Cognition Devin（parallel sessions 02-2026、Managed Devins 03、agent scheduling、coordinator/implementor/verifier agent 类型）＋Cognition "Don't Build Multi-Agents"（单线程+压缩，与 Anthropic 对垒）（12.6/12.7）。若直引对垒措辞回 cognition.ai 原帖。
- 【厂商实践】Cursor 3.0（04-02、Agents Window 汇总 active session、cloud VM 隔离）（12.2/12.5 背景）。
- 【规范/官方文档】Claude Code — background/foreground subagent、spawn 前预批工具权限＋运行时 auto-deny 未预批、独立 context window（12.4）。指针 code.claude.com。
...
- 【规范/官方文档】LangGraph — INVALID_CONCURRENT_GRAPH_UPDATE＋reducer＋乐观并发（base checkpoint id）＋高吞吐重试抖动；superstep 事务性＋RetryPolicy 重试前清 writes（12.5/12.6）。指针 research/volume2/05。
...
- 【预印】Cross-Agent Privilege Escalation（2025-09 演示）＋"有效权限=agent∩user 取交集"建议（CSA research note / WorkOS）（12.4）；单 agent 同等 token 预算反超多 agent（arXiv:2604.02460/2604.27891，量级参考）（12.3）。终稿回核出处与等级。
