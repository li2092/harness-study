...
| 规格 / 可执行规格 | 规格：语言无关的条文，写明数据结构、状态转移、事件 schema 与故障包判定标准，每条都能逐条执行判定；写成可运行的一致性测试（conformance test）时，才是严格意义上的可执行规格 | 第十四章 14.7 |
| 故障包通过数 | 标准故障包中已跑通的 fixture 数量，用来衡量构造进度；它衡量的是正确性，不等同于成熟度尺上的 L2 | 第十四章 |
| 热点交叉发现表 | 评审工具：以概念主线索引为线索，挑高风险的交叉点逐一检查，每行记一个交叉点、它在什么场景下出事 | 第十五章 15.2 |
| 场景卡 | 评审工具：一张卡写一个具体场景（谁、在什么时机、做了什么操作），列出它同时碰到哪几份契约、可能在哪里出事 | 第十五章 15.2 |
| 第一天智能水平 | core 仍按上线当天的模型能力与假设设计；模型换代后这些写死的假设逐条过时，而团队没有测量体系察觉，评审按高残余风险记 | 第十五章 15.5 |
...
| v2 | 2026-07-26 | 用户裁决两批。**一、概念主线索引回填至全卷**（第十至十五章共 14 站；主线五原"第十一章 主场（前向）"实指成 11.6 收口＋工作制品 U；六条收敛句尾同步扩写；维护规则加"落不上就照实留空"），并新增图 附.1 六条主线穿章图。**五、图表清单按方案 B 换工作，改名"五、配图导览"**：原节是出图前的工单形态（"正文各章的图当前均为（配图占位）""供成稿时统一制作"＋一条自指的 jimi-ink 清除指令），h2 之后的引用块管线剥不到、会原样印给读者；且与 PDF 自动图目录重复。改为逐图的用途索引——图号／档（核心 vs 补充）／它替你回答什么／对应哪个工作制品，共 17 行；顺带修掉旧表的 1.0 挂节、1.4 作废行、4.8"数据流向"、8.2 `outcome_verified` 四处失真。L3 自我描述"图在哪"→"哪张图答哪个问题"，覆盖声明同步 |
