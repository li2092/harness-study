...
三层的关系是单向的：系统正确保证状态正确，状态正确保证输出可信。反过来不成立——§一 1.0 引过的 MAST 数据已经给过量化版本：1600 余条故障轨迹里，三大类故障全部落在系统层【评审，arXiv:2503.13657】。盯着输出调 prompt，修的是最上面那层的抽样质量，下面两层原样不动。
...
一个旁证：腾讯 HY LLM Frontier 团队与高校合作、2026 年 7 月开源的 Harness Handbook 做的是反方向的事——把既有 harness 代码库自动整理成"行为单元"，每个单元的字段是 Trigger、Permission rule、State change、Execution path、Edge cases、Evidence【预印，arXiv:2607.13285，终稿前回核仓库现状】。他们从代码测绘实然，契约从需求声明应然，两边各自走到了高度同构的一组字段。这组字段是这类系统的自然坐标，用哪个方向走到它都一样。
...
对策是给判据自己建一套体检。统计学界把它叫 **sanity check 三件套**，出自 PCS（可预测性—可计算性—稳定性）框架【预印，arXiv:2604.11003】——入门卷 §七 已把它当"工作台怎么识别消化不了的数据"讲过一遍，本章换一个更前置的用途：给判据自己做体检。三件套是——Yes Check：同任务重复 N≥3，看结论是否稳定；Overlap Check：把期望值换成打乱的对照组，检查高分是不是泄漏换来的；扰动组：无关字段、匿名化、重排、正反诱导各来一遍，看结论掉多少。原研究团队用这套东西复检 11 个数据集，6 个的积极结论在 sanity check 下站不住——超过一半（"11／6"与判定口径终稿前逐字回核原文；本章开场讲的正是转述数字出错的教训，不给自己留同样的坑）。工程落点是报告里的三个强制字段：reliability（N 次重复的稳定性）、perturbation_drop（扰动后的跌幅）、leakage_risk（泄漏检查结论）。判据六要素登记的是系统的承诺，这三格登记的是判据自身的体检结果【经验】。
...
归因还要分维度，taxonomy 不能直接当结论。公开客服基准 τ-bench 有个任务实例：agent 把该退的 1628 美元算对了、也写进了数据库，只是没有在回复里说出来——单一 pass 指标把它记作彻底失败，与"数据库全改错"同罪【预印，τ-bench Task 7】。反过来，开场事故里 failure_taxonomy 显示一片"代码改动缺失"，实际是 verifier 统计口径错误。两个方向合成一条纪律：通过率之下必须有分维度的失败归因（工具参数／系统状态／沟通／流程四维起步），并且第一次建立基线时，失败案例必须人工深挖至少三个——静默的判据 bug 只有深挖抓得到【经验】。
...
模型侧有整套平行版本。Harness-Bench 给它起了名字：execution alignment failure——"看似合理的推理与工具反馈、工作区状态、证据或可验证的输出契约脱钩"，占其故障分类的 36.4%【预印，arXiv:2605.27922】；2026 年 6 月还出现了一小簇专门研究"自信收尾、静默失败"的预印本【预印，arXiv:2606.09863 等】。§一 开头那件怪事——agent 声称做完了、其实没做——就是模型版的第二层：它的推理在自己的上下文里完全自洽，只是与磁盘脱钩了。
...
论点讲完，最后把它摆回学术地图，看六契约与文献的切法怎么对上。学术侧把 harness 形式化为六元组 ℋ=⟨ℐ_obs, 𝒞, ℒ, ℐ_act, 𝒮, 𝒱⟩——观察接口、上下文管理、控制循环、动作接口、状态与产物存储、验证与治理【预印，arXiv:2606.20683，综述覆盖 110+ 论文、23 个系统】。这是**按组件**切分。本卷的六类契约**按保证**切分，一个契约横跨多个组件：真相 ≈ 𝒮+𝒞（context 是状态的投影）；转移 ≈ ℒ；副作用 ≈ ℐ_act；交互 ≈ ℐ_obs；权限与证据 ≈ 𝒱 的治理半边与验证半边。两种切法回答不同问题：组件切分回答"系统由什么构成"，保证切分回答"系统欠外界什么"。给出映射是为了与文献可对话，不是服从文献的切法。
...
- 【预印】PCS sanity check — arXiv:2604.11003。三件套；"11 数据集 6 结论"待终稿逐字回核（2.6）。
- 【预印】τ-bench — Task 7 的 1628 美元分维归因实例（2.6）。
- 【预印】Harness-Bench — arXiv:2605.27922。execution alignment failure 与 36.4%（2.9）。
...
- 【预印+厂商实践】Harness Handbook — arXiv:2607.13285。行为单元字段与契约六要素平行收敛（2.4）。
