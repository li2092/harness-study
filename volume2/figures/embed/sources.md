...
| 证据契约 | 凭什么相信状态的来路、动作和完成声明？ | 第十三章 | 全部四格的地基 |
...
| 图 | 档 | 它替你回答什么 | 接哪件 |
|---|---|---|---|
| 图 1.0 | 补充 | 入门卷行装与开场的真正问题——部件都在、个个都有点问题，两份研究结果把病根钉在部件之外 | 1.0 命题（工程质量是独立变量） |
...
## tb-14-3 · 14-build.md · 新增（2026-07-27 示范两步）

| 表 | 最小字段 | 字段来历 |
|---|---|---|
| runs | id、session_id、status（七值）、state_machine_version、terminal_reason | 第五章 5.2 七状态、5.4 终结原因、5.10 版本 |
| events | seq（自增，顺序的真相）、type、correlation 链（八级）、payload | 第六章 6.1 双轨与全局规则、第十三章 13.3 关联链 |
| effects | id、run_id、step_no、effect_no、action_class、idempotency_key、attempt_no、result_status、outcome_status | 第八章 8.2 四段账、8.4 四类动作、8.5 幂等 key |

## tb-15-3 · 15-review.md · 新增（2026-07-27 走查样张）

| 契约 | 触发场景 | 残余风险 | owner（角色） | 期限 |
|---|---|---|---|---|
| 真相契约 | 运行中会话被读取 | 只能读到 run 开始前的旧状态，用户误判"丢了"后重问 | 存储侧负责人 | 增量持久化上线前 |
| 转移契约 | 用户重问触发抢占 | 抢占决策无事件，旧 run 半轮无声丢失 | 内核负责人 | 转移表与随行事件就位前 |
| 交互契约 | 点击回答里的外链 | 整窗被带走，前端状态全部丢失 | 前端负责人 | opener 白名单上线前 |
| 证据契约 | 任何一次故障复盘 | 关键动作零运行时记录，复盘只能读代码重构 | 平台负责人 | 事件表 L1→L2 五条工单完成前 |
