...
## H · Terminology and casing reference

The Chinese edition's §H is a Chinese-English glossary. In this English edition it is repurposed as a terminology and casing reference — which terms stay lowercase in running prose, which are capitalized as proper names, and the few terms this book coins or clarifies.

### H.A · Core terms · lowercase in running prose

Used in their ordinary technical sense, lowercase: agent, context, context engineering, prompt, prompt engineering, tool, tool call, memory, cache, inference, reward, policy, orchestration, sandbox. **LLM** is uppercase ("large model" only where it reads naturally).

### H.B · Terms this book coins or clarifies

| Term | Note |
|---|---|
| observation surface | this volume's term; lowercase in prose |
| reasoning vs inference | reasoning = the thinking process, the model's internal chain-of-thought; inference = one model call (distinguished at the head of §5.1) · keep the two distinct |
| harness | the book's core term; lowercase in prose, capitalized only in names (Harness Study, Harness Lab, Harness Prompt) · on every AI-slop list, but kept deliberately |

### H.C · Terms kept in English, with a one-line gloss

| Term | Gloss |
|---|---|
| verifier | the component that rules PASS/FAIL on an output · one term only, never "validator" |
| trajectory | the event stream of one run |
| scratchpad | the model's working notes |
| artifact | a produced output / work product |
| hook | a deterministic callback fired on a lifecycle event |
| Skill | Anthropic's named capability · the Agent Skills open standard, 2025-10 |
| ACI | agent-computer interface |
| MCP | Model Context Protocol · Anthropic, 2024-11 |
| RAG | Retrieval-Augmented Generation · Lewis et al., 2020 |
| ReAct | the Reason + Act paradigm |
| agent loop | the agent's main loop (titled "Agent Loop" per the chapter) |
| episode | one task / one run (RL term) |
| rollout | one sampled run (RL term) |
| tool schema · patch · diff · fork | left untranslated, kept in their technical sense |

### H.D · Vendor / product / person names · verbatim

- **Vendors**: Anthropic / OpenAI / Google / DeepSeek / Moonshot / Meta / Microsoft
- **Products**: Claude / GPT / Gemini / Codex / Cursor / Kimi / LangChain / LangGraph
- **People**: Karpathy / Schulman / Qian Xuesen / Norbert Wiener, and others
- **Generic computing terms**: API / JSON / SSE / HTTP / REST, and the like, untranslated
