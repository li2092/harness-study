...
Terms already explained in §I–§IV and §5.1–§5.7 (schema, the verifier concept, trajectory, observation, artifact, ablation, the general concept of reward hacking, and so on) are not repeated below. Only the terms making their first appearance in §5.8 are listed.
...
- **control groups (overlap_positive / overlap_negative)**: the method the author's project uses to check for answer disclosure. One set of positive and one set of negative control tasks calibrate the answer-disclosure leakage check (see §5.8.7).
