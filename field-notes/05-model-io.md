...
同一个工具的参数 schema 会因为模型的小版本更新而需要分叉：不是大版本，而是同一条模型线相隔不到两个月的两个版本，字段定义要各写一套。另一个商用实现里有现成的例子：读文件工具在早一个版本里叫 read_file，路径字段叫 target_file，允许相对路径；晚一个版本改叫 Read，字段叫 path，只收绝对路径。待办工具连字段顺序和"内容是否必填"都不一样。代码里没有写为什么要分开，推断是两个版本训练时见到的工具定义不同，下发的定义要和各自训练时的一致。
