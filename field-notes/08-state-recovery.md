...
第十三章的复盘把这个缓存列为负贡献的一例：为性能加的机制带来了一致性故障，收益远小于代价。<!-- 待作者补充：这一例是消融实验测出来的（若是，用的是什么指标），还是工程复盘的判断？第十三章 13.3 需要同步措辞。 -->
...
- 去重用的随机数，两种情况都不带。它由进程生成，用来识别重复请求；把旧值带进新进程，新进程发出的正常请求可能撞上旧记录，被误判成重复。<!-- 待作者补充：确认误判的具体机制是否如此。 -->
