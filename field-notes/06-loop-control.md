...
第一道防线在工具层：拒绝时直接指出错的字段和正确的名字（第四章 4.4），模型多半下一次就能改对。

第二道防线是计数，只数三种确定性的拒绝：参数非法、不符 schema、工具不存在。这三种拒绝的原因在调用本身，参数不改，重发多少次结果都一样。做法是按"工具名加参数指纹"记录被拒次数，同一调用被拒超过三次（经验值，按场景调整），从第四次起在拒绝结果上叠加换策略的指引：修正参数、改用其他工具或方法，或者向用户说明卡在哪里。这个调用一旦通过全部检查进入执行，计数清零。参数指纹要与键的顺序无关：先把参数对象按键名排序再取哈希，`{a, b}` 和 `{b, a}` 才会算成同一次调用。

其余拒绝不计数：用户跳过、hook 拦截、策略拦截、验证未通过、需要补充信息、上下文告警。这几种的原因在模型之外，下一刻就可能解除：用户可能改主意批准，先读一遍文件就能让验证通过，策略也可能更新。早期版本对这九种拒绝都计数，后来收窄到上面三种，第二章 2.2 记了原因：对外部原因造成的拒绝喊"换策略"，是 harness 在替外部条件下结论。

计数的处置只是换文案，不拦截调用。各道检查照常先跑，不能根据计数提前判定"这次也会被拒"而跳过检查：拒绝原因一旦解除，调用就该放行。业界常把这类做法叫熔断（circuit breaker），这里只借用了熔断器"打开"这一步，而且打开之后只换文案：没有冷却一段时间后再半开试探的过程，也不强制终止。

**检测重复要看两样：参数有没有变，结果有没有变。** 参数不变的重试是卡住，参数在变但结果一直失败的是在瞎试。两种都值得在日志里分开记录，供事后诊断；运行时自动介入的只有确定性拒绝这一类。
...
3. **上下文不能全量同步。** 把主 agent 的完整历史广播给每个子 agent，token 按 agent 数成倍增长，一份错误的中间状态也会传给所有人。一次实践最后收敛到只同步两样东西：固定格式的任务状态，和每个阶段完成时写下的过程摘要。子 agent 在自己的上下文里工作，结果浓缩后交回主 agent 校验。近期一项研究给了同向的数（arXiv 2606.21666，2026-06）：全量广播比不同步的幻觉率还高三成，改为交换压缩后的状态摘要就降回去，调用次数也少一半以上。这项研究样本不大，作者也说明这一效应只在旅行规划类任务上出现，在软件规划任务上没有复现。

入门卷 §5.1 给了成本侧的理由。Anthropic 在介绍其多智能体研究系统时给出过一组数字（anthropic.com/engineering/multi-agent-research-system，2025-06）：相对普通对话，单 agent 约用 4 倍 token，多 agent 约 15 倍。两数相除，多 agent 大约比单 agent 再贵三到四倍，因此什么时候上子 agent 是高门槛决策。近期一项受控研究（Google 等，arXiv 2512.08296）把这道门槛说得更具体：单 agent 已经能做到四成五以上的任务，再加协调多半是负收益；能拆成并行子任务的收益大，顺序型任务掉得最多，工具密集的任务被协调开销拖累；没有中心校验的架构，一个 agent 的错误会放大十几倍传到结果里。
