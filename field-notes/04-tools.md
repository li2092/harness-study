...
模型由此只知道工具做什么，字段细节靠常识推测。一次真实的表现是：接口要求 `total_baggages`，模型传 `baggages`；接口要求 `insurance`，模型传 `travel_insurance`。连续四次调用失败才试对，每次失败消耗一轮，而这四轮里模型还会顺带修改其他本来正确的参数，把失败原因混在一起。

试过的无效做法是把字段规范补进 system prompt。它失效的原因和第一章步骤 5（注入提醒）相同：规则离使用它的时刻越远，被遵循的概率越低。system prompt 在上下文的最前面，与具体工具之间没有绑定；模型在第 20 轮挑工具、填参数时，参考的是眼前的工具清单。

有效的位置是 description 本身：直接列出必填字段名、类型、枚举值。实测这条改动比改 system prompt 有效得多，而成本只是改几行文本。<!-- 待作者补充："有效得多"（原稿"一个量级"）用的是什么指标（调用失败次数、试错轮数还是任务通过率）、对比基线和样本量？ -->
...
目标工具协议往往只接受 JSON Schema 的一个子集。转换代码按白名单过滤关键字时，把 `$defs` 段整体删掉了，`$ref` 却原样保留，模型看到的就是指向空处的引用。后果不是报错，而是这批工具集体不可用：凡是依赖那些定义的工具，模型都拼不出正确参数，反复尝试，反复失败。

这个故障最难的地方在归因。表面现象是"模型的工具调用能力突然变差"，几个写工具全部失效而读工具正常，很容易被判成模型能力问题，进而做出换模型、调 prompt 这类无效动作。真正的原因在一行 schema 转换代码里。

一次实测中，修复这一项让任务通过率比修复前提高了 10 个百分点，模型侧未做任何改动。<!-- 待作者补充：这次实测的评测集、任务数、每个任务的重复次数和所用模型？ -->
...
入门卷 §5.3.4 给了更完整的要求：schema 规范化要快速失败（fail-fast），内联引用、显式标注可选字段、对齐枚举类型，任何一步不完整，就拒绝注册这个工具。开启模型侧严格模式（strict，服务端用约束解码保证模型输出符合 schema，只支持 JSON Schema 的一个子集）时，残缺的 schema 会直接被服务端以 400 拒绝；本节的静默失败出现在非严格模式。
...
对缺失参数，有一个看起来体贴的做法：自动填入默认值，让调用成功。这个机制的问题要等到第十三章才能讲清楚：在消融实验（关掉某一个机制、其余不变，比较评测指标的变化）里，它是负贡献，关掉之后通过率反而上升（上升幅度本身不显著，依据还包括对失败轨迹的检查，见 13.3）。原因是它遮蔽了模型本该收到的错误信号。
...
与之相对，还有一条要求：**拒绝时给出的建议必须是模型能执行的。** 有过一次实际的死锁：写一个本会话没读过的文件会被拦下，拦截文案要求先读一遍，而这次运行里读写记账器没有记录读操作，于是模型照做，读完再写，仍然撞上同一句拒绝。<!-- 待作者补充：当时记账器为什么没有记到这次读操作（记账器不可用、读走了不经过记账器的路径，还是其他原因）？ --> 处理办法有两点：
