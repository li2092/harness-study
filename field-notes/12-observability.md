...
- 用例分成开发集与留出集（held-out set）。开发集用于日常调参；留出集不据它改东西，每次改动前后都跑，只用来检查开发集上的提升是否还在。
...
全程用基准官方推荐的模型，迭代成本压不住：一轮评测的模拟用户开销可能比被测系统还高。全程用便宜模型，测出的成绩别人不认，因为模拟用户的追问强度不同，分数没有可比性。

有效做法是分双轨：内部迭代用便宜模型（一个任务几分钱人民币，只算模拟用户一侧），验证工程改动是否有效；对外报告成绩用官方推荐的模型，保证与他人可比。第十三章 13.2 的两次自测都属于内部迭代，没有用官方推荐的模拟用户。便宜模型能验证的是与模型能力无关的改动，即协议、工具、压缩、恢复这几类；prompt 措辞与规则遵循类改动的效果随模型变，这类在目标模型上抽样验证。
...
有效做法是每次启动前写下本轮要验证什么。入门卷 §7 还要求先做一步：估算统计功效（statistical power）。具体是算出在当前样本量（任务数乘重复次数）下的最小可检测效应（minimum detectable effect，MDE），即在给定显著性水平与功效下能可靠分辨的最小差异。样本量越小，MDE 越大：大的差异看得出来，小于 MDE 的差异会淹没在噪声里。以五十个任务、每个任务运行一次为例，在 5% 显著性水平和 80% 功效下，最小可检测效应约二十个百分点；要压到十个百分点左右，每个任务至少要重复四次。 不先算这笔账，小样本消融的典型结局是把噪声当信号：同一个机制的贡献值在两轮之间符号翻转。
...
前五条出自同一次排查。那次排查期间，被测系统的通过率一路升到 92%，模型一直没换（通过率的来源见第十三章 13.2）。排查记下的八个非常规问题全部出在模型之外，分布在评测管线、运行环境与协议转换上；最后仍未通过的四个任务里，只有一个明确归因于模型本身，另外三个当时没有归因。 **评测出现批量的异常失败时，排查顺序是先评测系统、再 harness、最后模型。**
