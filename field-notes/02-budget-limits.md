...
有一种看起来自然的补救，是把剩余步数报给模型，让它自己安排。上线后的使用观察否定了它，连同另一个同源的做法：按计数拦模型的工具调用，例如同一文件连改三次没有重读就拦下，重复被拒之后由 harness 喊"换策略"。

两者并存期间，模型的能力和主动性被明显压住：该继续探索的地方停下来，该多试一次的地方退让，产出停在保守的一版。播报的计数还会过期、互相矛盾，第七章 7.3 记了它从上线到删除的全过程。两者后来都撤了，拦截收窄到三种确定性的拒绝：参数非法、不符 schema、工具不存在。其余拒绝的原因都在模型之外，harness 不该替它下结论。
...
**真正的失控靠专门的检测来防**，不靠步数：连续多轮零工具调用（空转，见第六章 6.5）、同一调用反复被确定性拒绝（第六章 6.1）、无进展的循环（第六章 6.2）。这些判据能识别真正的失控，而步数上限识别不了：它只知道跑了多久，不知道有没有在推进。
...
*图 2.2 · 步数上限按最坏情况设：撞上限是硬停，提前收尾与它无关*
...
*图 2.3 · 直接截断、结构化摘要、外置引用与直接拒绝的不同结果*
...
三项相加约两万 token 上下，这是典型的一轮。预留要按最坏的一轮留。推理 token 计入输出上限，推理与输出共用这一个额度，所以最坏的一轮是输出上限加一轮工具结果，不必把三项分开再加。一次实践在 200K 窗口上的配置是：输出上限 30K；单条文件读取的结果上限四万字符，约合一万到两万七千 token；一轮所有工具结果合计不超过压缩触发线的四分之一，按那次的触发线约两三万 token。两项加上几千 token 余量，约六七万，三成出头。两成（40K）装不下这样的一轮；五成（100K）又让历史只剩一半，压缩过于频繁。三到四成（60K 到 80K）落在两者之间。

这个比例有个前提：服务端把输出计入窗口。2.1 要求输出上限给足，而部分 200K 窗口的模型最大输出有 64K 甚至 128K，照此预留会远超三到四成。有的服务窗口只限输入、输出另算；也有的服务接受"输入加输出上限超过窗口"的请求，生成到窗口边缘才停。这两种情况按预期的实际生成量预留即可。

**预算的分母是窗口，分子不止是历史。**

![](../diagrams/fn-layered-02-window.png)

*图 2.4 · 窗口的两部分分配与最坏一轮的估法*
