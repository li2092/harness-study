...
实现层在 2025 年还多了一个 provider 侧的新成员：Anthropic 2025-09 在 API 层推出 [context editing](https://platform.claude.com/docs/en/build-with-claude/context-editing)（按配置的策略自动清理早期 tool result）和 memory tool（跨对话记忆——Claude 通过工具读写，存储落在开发者自己的后端）——前者就是 micro-compact 这件抽象功能被做进了 API，后者是 Memory 读写职能的工具化。按"件 vs 实现"的框架归位即可：平台把件做进 API 不改变件本身的职责划分，清什么、留什么、什么时候清这些决策仍然是 harness 的事——执行可以交给 provider，判断不能交出去。
