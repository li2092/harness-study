...
**Phase B 单点消融的价值不只在"量化正贡献"，更在于能抓出"隐性的负贡献机制"。** 一个真实例子：某 harness 有个"工具参数自动补全"机制，模型调用工具缺字段时自动填默认值，让调用不至于报错。代码逻辑完全正确，单元测试全过，看起来是个周到的容错设计。但在端到端单点消融里关掉它，通过率不降反升：它**把模型本该暴露的参数错误悄悄盖住了**。模型传错参数，工具靠补全照样跑出一个"看着对、其实不对"的结果，模型收不到报错，也就不会自我修正，一路错下去。关掉补全后，错参数的调用直接失败，模型收到报错，重新构造参数，做对了。这种"局部正确、全局有害"的机制，代码评审看不出，单元测试也测不到（单测测的是代码逻辑对不对，而不是它在真实系统里有没有用），只有在真实模型、真实工具链、真实任务上做单点消融才能暴露出来。这正是 Ablate 层比"凭感觉调机制"多出来的一步。
...
- **开发集与留出集分开**：日常调 prompt、规则、工具描述只看开发集；留出集只在上线前跑，不根据它做调整；
