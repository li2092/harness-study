...
三卷共用的术语约定与外部事实的核查结论，见仓库根目录的 [`术语对照表.md`](../术语对照表.md)。英文版（`introduction.en/`）已同步本版修订。
...
| §5.9 | Codex Auto-review 判为有害就拒绝 | 判为不确定或有害都转交人工审批，与 §5.9 后文"软检查"一段一致；图 5.24 随之改 |
| §5.11 | 先 push main 再提 PR；死锁因 Handler 自带锁 | 先推功能分支再向 main 提 PR；死锁因同一把不可重入锁被重复获取（Handler 自带的锁本是 RLock） |
| §5.11 | Turn 16 的 Safety 走查按权限模式 → 规则 → hook 的顺序写 | 按 §5.9.2 的求值顺序改写：PreToolUse hook 先于规则，ask 规则命中后权限模式不再参与判定，沙箱在 push 执行时检查；Turn 2、Turn 9 与图 5.29 随之改 |
| §7 | HPO 的 acquisition function（如 Hyperband） | 采集函数属于贝叶斯优化 / TPE，Hyperband 用 successive halving |
| §7 | 开头称 AHE、Meta-Harness 提出了 Ablate 方向 | 与 §7.7 统一：两篇论文覆盖 Tune 和 Iterate 的一部分，作者没有看到把消融做成工作台功能的产品；图 7.3 随之改 |
...
| §5.4.3 Artifact · 跨 run 永久产物 | 跨 run 长期产物（Artifact 直到显式删除才消失） |
| §5.6 三层定位（自我演化基础 P0、运行时反馈 P1、进阶层） | 两个作用加一个案例；优先级按 §5.0 的定义改为运行时反馈 P0、跨 run 数据来源 P2 |
...
- 本卷 PDF 已按本版重新构建（0926 版），封面与内页版式同时更换。
- 中文配图按 PDF 里的实际尺寸重新排版：图内正文由约 5.6pt 放大到约 9pt，底色与内页纸色统一，图中文字不变。英文版配图沿用原版式。
