...
这两处不同导致 harness 比 OS 多了一类基础职能：**防错**。OS 不需要假设 CPU 会突然抽风给错答案，但 harness 必须假设 LLM 每一步都可能犯错。这就是为什么 harness 必须有两道 P0 闸门：**verifier**（验证器，防模型输出错的——8 件 runtime 机制中的一件）和 **tool policy**（工具策略，防工具调用错的——它不单列成件，由 Tool Registry 的 policy 字段和 Safety 控制面共同承载）——一道在输出侧拦截"假完成"（模型说做完了但实际没做对），一道在输入侧拦截"乱调用"（模型想调一个不该调的工具或带不该带的参数）。这两道闸门在 OS 设计里几乎找不到对应物，是 LLM 这种概率性核心被工程化的标志——读者先在概念地图上把它们的位置标好：verifier 是输出闸门，tool policy 是输入闸门。
