...
*图 1.1 · 模型能做的与任务要的之间，harness 填的那道缝*
...
短句里每个词都不是装饰。`runs tools` 说明动作面是工具调用，不是纯文本生成；`in a loop` 是关键——不是 `calls tools`（一次性调用），而是循环。循环这个词暗示了**多轮**：每轮模型看到上一轮的结果再决定下一步。循环暗示了**状态**：要知道循环是否该终止，得有某种"已经做了什么"的状态记忆。循环暗示了**终止条件**：得有判定"目标达成"的机制。Willison 这短短一句的定义把"为什么 agent 需要工程脚手架"的种子全埋了——只是他没展开讲是什么样的脚手架。
...
这两处不同导致 harness 比 OS 多了一类基础职能：**防错**。OS 不需要假设 CPU 会突然抽风给错答案，但 harness 必须假设 LLM 每一步都可能犯错。这就是为什么 harness 必须有两道 P0 闸门：**verifier**（验证器，防模型输出错的——8 件 runtime 机制中的一件）和 **tool policy**（工具策略，防工具调用错的——它不单列成件，由 Tool Registry 的 policy 字段和 Safety 控制面共同承载）——一道在输出侧拦截"假完成"（模型说做完了但实际没做对），一道在输入侧拦截"乱调用"（模型想调一个不该调的工具或带不该带的参数）。这两道闸门里，verifier 在 OS 设计里几乎找不到对应物——OS 从不需要验证 CPU 有没有谎报"算完了"；tool policy 倒有一个远亲，OS 的权限体系和 syscall gate（§5.9 讲 Safety 控制面时会借这个同构展开），但 OS 拦的是"没权限"，tool policy 还要拦"有权限但这一步不该做"。这是 LLM 这种概率性核心被工程化的标志——读者先在概念地图上把它们的位置标好：verifier 是输出闸门，tool policy 是输入闸门。
