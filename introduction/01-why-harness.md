...
要把 harness 是什么这件事讲清楚，最好的办法不是给一个权威定义，而是看三个 2025-2026 在业界被广泛接受的定义怎么递进。
...
**第一处不同 · 确定性 vs 概率性配合**。OS 跟 CPU 是确定性配合。CPU 拿到一条 `MOV` 指令永远做"加载寄存器"这件事，永远不会"瞎跑"成 `ADD` 指令，也不会"幻觉"出一个不存在的内存地址。OS 调度 CPU 时心里有数——给什么指令做什么事，结果可预测。但 harness 跟 LLM 是**概率性配合**。同一份 prompt 跑两次，LLM 可能给两个不同结果；模型可能在每一步编造一个不存在的工具名字、可能在本来应该用 A 工具的场景里选了 B、可能"忘了"三轮前自己已经做过的事。OS 写出来是给确定性器件用的，harness 写出来是给概率性器件用的——这一字之差，让 harness 的工程难度比 OS 高出一整个维度。
...
这两处不同导致 harness 比 OS 多了一类基础职能：**防错**。OS 不需要假设 CPU 会突然给错答案，但 harness 必须假设 LLM 每一步都可能犯错。这就是为什么 harness 必须有两道 P0 闸门：**verifier**（验证器，防模型输出错的——8 件 runtime 机制中的一件）和 **tool policy**（工具策略，防工具调用错的——它不单列成件，由 Tool Registry 的 policy 字段和 Safety 控制面共同承载）——一道在输出侧拦截"假完成"（模型说做完了但实际没做对），一道在输入侧拦截"乱调用"（模型想调一个不该调的工具或带不该带的参数）。这两道闸门里，verifier 在 OS 设计里几乎找不到对应物——OS 从不需要验证 CPU 有没有谎报"算完了"；tool policy 倒有一个近似对应物，OS 的权限体系和 syscall gate（§5.9 讲 Safety 控制面时会借这个同构展开），但 OS 拦的是"没权限"，tool policy 还要拦"有权限但这一步不该做"。这是 LLM 这种概率性核心被工程化的标志——读者先在概念地图上把它们的位置标好：verifier 是输出闸门，tool policy 是输入闸门。
...
**一端是 coding agent，它的 harness 必须是个半成品**。每个程序员的项目架构、代码规范、工具链、连审美都不一样，没有哪套预设能替所有人适配好；好的 coding harness 恰恰要把 hook、配置、CLAUDE.md、skill 这些扩展点留出来，让每个程序员把自己的工程语境注入进去。一个什么都替你定好、不留适配空间的"完成品" coding agent，反而是个坏 harness——它把本该由程序员填的那部分锁死了。**另一端是垂域 agent，它该是个成品**。医疗、法律、民航某条具体流程，领域本体固定、合规边界硬、用户既不该也不能随意改流程，这时 harness 要把领域规则、状态机、操作集都锁进去（后面副 harness 那章讲的 5 维度本体正是干这件事），留太多扩展点反而是风险。**办公向 agent 落在中间、偏成品**——写邮件、排日程、做表格这类场景比写代码收敛得多，开箱即用加少量配置就够。这条光谱的根源不在产品成熟度，而在领域本身：领域越发散、跨场景差异越大（coding 是极端），harness 越要开放、把适配权交出去；领域越收敛、合规约束越强（垂域是极端），harness 越要固化、把适配权收回来。判断自己要做的 agent 该做成什么样，先问这个领域落在光谱的哪一端。
