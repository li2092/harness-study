...
四种 routing 决策可以独立运作也可以组合。生产 harness 里 routing 模块通常是一个独立的、可配置的策略层，里面是一组规则——"如果 X 触发则切到 provider Y" 这种声明式配置，让 routing 行为不写死在代码里、可以通过配置文件调整。routing 的决策方式要分档看。确定性要求高的档——failover、权限、预算——必须用代码：这些决策错一次就是事故，且每次推理前都要做，用 LLM 来做会引入新的不确定性，延迟和成本也都不划算。难度/成本这一档可以引入轻量的 learned router——用一个小分类器判"这个请求走便宜档还是强档"，RouteLLM（LMSYS，2024 开源）验证过这条路，ChatGPT 的 GPT-5（2025-08）在 fast/thinking 两档之间的实时分流就是它的平台级版本；约束条件是三件——决策理由进 trace 可审计、可离线回放、延迟可控。"routing 用代码不用 LLM" 作为默认起点仍然成立：learned router 是流量和数据攒够之后的升级档，不是起手式。
