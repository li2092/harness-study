...
failover 还要把一条边界说死：**它重发的是 completion 请求，completion 本身没有副作用，可以放心重试**；但同样的 retry 思路搬到工具执行层就危险了——一个写操作工具在"已执行、未返回"的窗口里超时，重试就是双重执行（扣两次款、发两封邮件）。所以 retry 策略必须分层：completion 层按上面四类触发条件自由重试；工具层的重试要么有幂等保障（给每次工具执行配请求指纹，重放前先查执行记录），要么对外部副作用类工具直接 fail-fast 转人工。这条线的落地在 5.3 的 ToolPolicy 里。
...
四种 routing 决策可以独立运作也可以组合。生产 harness 里 routing 模块通常是一个独立的、可配置的策略层，里面是一组规则——"如果 X 触发则切到 provider Y" 这种声明式配置，让 routing 行为不写死在代码里、可以通过配置文件调整。routing 的决策方式要分档看。确定性要求高的档——failover、权限、预算——必须用代码：这些决策错一次就是事故，且每次推理前都要做，用 LLM 来做会引入新的不确定性，延迟和成本也都不划算。难度/成本这一档可以引入轻量的 learned router——用一个小分类器判"这个请求走便宜档还是强档"，RouteLLM[^routellm-2024]（LMSYS，2024 开源）验证过这条路，ChatGPT 的 GPT-5（[2025-08 发布](https://openai.com/index/introducing-gpt-5/)）在 fast/thinking 两档之间的实时分流就是它的平台级版本；约束条件是三件——决策理由进 trace 可审计、可离线回放、延迟可控。"routing 用代码不用 LLM" 作为默认起点仍然成立：learned router 是流量和数据攒够之后的升级档，不是起手式。
...
有一类高频又难查的现象——agent 该调工具却没调，trajectory 里那一回合只有一段文本、没有 tool_call。直觉先怪模型能力或 prompt，但实测里成因常在 Adapter 这条接缝上，分三种，对策完全不同。

**第一种 · 模型其实调了，但调用写在正文文本里、没进结构化字段。** 有些模型（尤其部分国产模型、以及被长 prompt 带偏时）会把工具调用以文本标签形式吐出来——直接在 content 里写 `<tool_call>name<arg_key>k</arg_key><arg_value>v</arg_value></tool_call>`，而不放进 API 的结构化 `tool_calls` 字段。Adapter 的响应解析若只认结构化字段，这种调用会被当普通文本丢弃，表面看是"没调工具"，实则**假阴性**。对策是解析层在结构化字段之外再兜一道文本标签的正则提取——tool call 的输出形态不是只有结构化一种，各家差异很大，解析层不能假设。

**第二种 · 模型真没调，因为请求侧没逼它调。** 默认 `tool_choice: auto` 是"可调可不调"，模型觉得能直接答就直接答。某些回合你明确需要它走工具（必须查库、必须落盘），就在请求侧把 `tool_choice` 设成 `required`/`any` 或指定具体工具。这是 Adapter 拼请求时的一个开关，也是"防不调"最直接的手段。但有边界——长期强制 `required` 会逼模型在不该调时硬调、制造噪声调用，所以它是**按回合按场景开**的策略，不是全局常开。

**第三种 · 模型真没调，因为 prompt 把它带偏了。** 一个反直觉但实测反复出现的现象——**过长的中文 prompt 会让某些模型"懒得"调工具、直接用文本作答**。触发工具调用的指令裹在一大段中文说明里时，部分模型会忽略工具直接回答。成因在 prompt 装配侧（属 Prompt Assets 机制范畴），表现却落在 Adapter 这层。对策是让"要触发工具调用"的指令短而靠前、长说明拆分，别让关键指令淹没在长上下文里。

三种成因对应三个层——响应解析、请求参数、prompt 装配——查"模型没调工具"时按这三处依次排除，比反复改 prompt 或换模型快得多。
...
这个常见误区的代价：任何 harness codebase 里如果允许 openai / anthropic SDK 直接被业务代码 import，模型 API 一升级（每年 1-3 次大版本）都会引发大规模 codebase 修改。2023-2026 期间 OpenAI function calling 改名 tool calling、Anthropic tool use 字段结构调整、各家 prompt caching 陆续加进来这几次升级里，没有 adapter 边界的项目每次都要改动大量散落的业务调用点，有 adapter 边界的项目只需改少数几个 adapter 文件。
