---
title: Harness Study · 智能体的工程实践 · 入门版
title_en: Harness Study · The Engineering Practice for AI Agents · Intro
slug: what-is-harness
version: 0.4
status: 整本写完 · 已按章节拆分 · 配图已嵌 · 等终审
last_updated: 2026-06-02
audience: AI PM / AI 业务人员 / 学习者 / 给 AI 看
structure: 按章节拆分 · 每章一个 md · §五每个细节一个 md（原 01-introduction part1–5 → introduction/）
---
...
本卷按章节拆分，每章一个文件；第五章（必备机制）每个细节一个文件。建议按顺序读，每个文件 frontmatter 里有 `prev` / `next` 衔接链。
...
**给 AI 看**——AI agent 自己读本卷做下游决策。推荐路径：按各文件 frontmatter `prev` / `next` 链依次读。不要跳读机制描述段——那是 prose 主体，跳了就只剩名字。
