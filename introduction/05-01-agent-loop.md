...
**进阶方向相关（5.1.7）** —— **learning by doing**（做中学 · 没有标定数据的开放域里智能的最后来源 · 对应 Silver & Sutton "经验时代"的核心主张）。**grounded rewards**（接地 reward · 从环境经验里接出来的 reward 信号（执行反馈 / 错误率 / 用户后续行为）· 与"来自人类预判的 reward"相对）。**MCTS**（Monte Carlo Tree Search · 蒙特卡洛树搜索 · selection → expansion → simulation → backpropagation 四步循环的树状搜索 · AlphaGo 一系的核心算法 · 5.1.7 强调它的核心是 backpropagation 需要的节点价值信号而不是树本身）。**learning progress**（学习进度 · "我在这个目标上还在进步吗"的元认知信号 · 用来给自生成目标排优先级 · 自动剔除不可能与已掌握两端）。**pass@k**（采样 k 次至少命中一次的通过率 · k=1 看单发命中 · k 大看模型分布里有没有正确路径）。
...
[^era-of-experience]: Welcome to the Era of Experience · David Silver, Richard S. Sutton · Google DeepMind · 2025 · MIT Press《Designing an Intelligence》书章预印本——经验时代四支柱：经验流（而非短交互片段）/ 行动观察接地于环境（而非仅人类对话）/ reward 接地于环境经验（而非人类预判——原文判断依赖人类预判 "usually leads to an impenetrable ceiling on the agent's performance"）/ 对经验做规划推理（而非仅以人类语汇推理）
[^voyager]: Voyager: An Open-Ended Embodied Agent with Large Language Models · Wang et al.（NVIDIA）· NeurIPS 2023 · arxiv 2305.16291——automatic curriculum（按当前能力提出"难一点"的下一个目标）+ 不断生长的可执行技能库，探索产物沉淀为跨任务复用技能的最早完整样板；其自验证依赖 Minecraft 可判定的世界状态，搬到无 ground truth 的开放产出域时这层保障会变弱
[^pass-at-k]: Does Reinforcement Learning Really Incentivize Reasoning Capacity in LLMs Beyond the Base Model? · OpenReview 4OsgYD7em5 · 2025 在审——发现 RLVR 模型在小 k 时胜过 base model、大 k 时反被反超，提示这类训练把概率质量集中到已有正确路径、以输出多样性为代价。有反驳认为结论受 k 取值与算法选择影响，非定论——此处只取其方向性提示：开放任务的发散阶段不要用收窄分布的策略
[^genrm-poll]: 节点软价值的两个已验证部件——Generative Verifiers: Reward Modeling as Next-Token Prediction · Google DeepMind · arxiv 2408.15240（把 verifier 做成先生成验证推理链再下判定的生成式模型 · 可多链投票）；Replacing Judges with Juries · Cohere · arxiv 2404.18796（多个不相交模型家族的小模型组成评审团 · 判定比单个大 judge 更准且成本低 7 倍以上）
[^magellan-alp]: MAGELLAN: Metacognitive predictions of learning progress guide autotelic LLM agents in large goal spaces · Gaven et al.（Inria Flowers 团队）· ICML 2025 · arxiv 2502.07709——agent 在线预测自身各目标上的胜任度，以绝对学习进度 ALP = |C_t(g) − C_{t−N}(g)| 作为目标采样优先级；在合成文本环境验证，未在生产级开放任务落地
