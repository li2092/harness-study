...
- **对照组（overlap_positive / overlap_negative）**：作者项目中检查答案明示的做法，用一组正例和一组负例校准一个确定性的泄漏检测器（见 5.8.7）。
...
**第四类是偏好泄漏**[^preference-leakage]：judge 与 agent 模型训练数据的生成模型有关联，导致系统性偏好，5.8.3 已经详细展开。对策是让 judge 与 agent 不用同一模型家族，最好跨厂商（第十一章落地 Spec 的 Outcome Judge 就这样要求）；多 judge 投票时，各 judge 也要来自不同家族。
